...
These standards apply to Python code in `kaos-mcp`.

`kaos-mcp` publishes the `kaos_mcp` import package. This package publishes CLI entry point(s): `kaos`, `kaos-mcp`.

## Package Shape

- Keep the import package name aligned with the distribution name:
  `kaos-mcp` publishes import package `kaos_mcp`.
- Declare the public API in `kaos_mcp.__all__` when the package has
...
- Names exported from `kaos_mcp.__all__`.
